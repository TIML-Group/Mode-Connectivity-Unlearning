...
class _TaskVector(abc.ABC):
    def __init__(
        ...
        if vector is not None:
            ...
            with torch.no_grad():
                # pretrained_state_dict = clear_ckpt(torch.load(pretrained_checkpoint))
                # finetuned_state_dict = clear_ckpt(torch.load(finetuned_checkpoint))
                ...
                for key in finetuned_state_dict:
                    # isinstance(pretrained_state_dict[key], torch.Tensor) and
